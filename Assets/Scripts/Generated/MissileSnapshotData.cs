...
public struct MissileSnapshotData : ISnapshotData<MissileSnapshotData>
{
    public uint tick;
    private uint ActiveComponentIsActive;
    ...
    public bool GetActiveComponentIsActive(GhostDeserializerState deserializerState)
    {
        return ActiveComponentIsActive!=0;
    }
    public bool GetActiveComponentIsActive()
    {
        return ActiveComponentIsActive!=0;
    }
    ...
    public void SetActiveComponentIsActive(bool val)
    {
        ActiveComponentIsActive = val?1u:0;
    }
    ...
    public void PredictDelta(uint tick, ref MissileSnapshotData baseline1, ref MissileSnapshotData baseline2)
    {
    ...
    public void Serialize(int networkId, ref MissileSnapshotData baseline, ref DataStreamWriter writer, NetworkCompressionModel compressionModel)
    {
        changeMask0 = (ActiveComponentIsActive != baseline.ActiveComponentIsActive) ? 1u : 0;
        ...
        if ((changeMask0 & (1 << 0)) != 0)
            writer.WritePackedUIntDelta(ActiveComponentIsActive, baseline.ActiveComponentIsActive, compressionModel);
    ...
    public void Deserialize(uint tick, ref MissileSnapshotData baseline, ref DataStreamReader reader,
        NetworkCompressionModel compressionModel)
    {
        this.tick = tick;
        changeMask0 = reader.ReadPackedUIntDelta(baseline.changeMask0, compressionModel);
        if ((changeMask0 & (1 << 0)) != 0)
            ActiveComponentIsActive = reader.ReadPackedUIntDelta(baseline.ActiveComponentIsActive, compressionModel);
        else
            ActiveComponentIsActive = baseline.ActiveComponentIsActive;
        if ((changeMask0 & (1 << 1)) != 0)
        {
    ...
    public void Interpolate(ref MissileSnapshotData target, float factor)
    {
